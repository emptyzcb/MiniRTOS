 ...
 *   - FPU context save/restore in PendSV (S16-S31)
 *   - EXC_RETURN value 0xFFFFFFFD (return to Thread mode, PSP)
...
/* Cortex-M4: 4 priority bits */
#define CORTEX_M4_PRIO_BITS     4
...
/*
 * Enable the FPU by setting CP10/CP11 to full access in CPACR.
 * Must be done before any FPU instruction is executed.
 */
                                ...
                                uint32_t stack_size)
{
    ...
    *(--sp) = 0x04040404UL;                 /* R4 */
    ...
    /*
     * Note: FPU registers (S16-S31) are NOT pre-initialized for new tasks.
     * They will be saved/restored by PendSV_Handler only if the task
     * has actually used the FPU (lazy stacking via EXC_RETURN bit 4).
     */
...
/* PRIMASK-based critical sections: global interrupt disable */
uint32_t os_port_enter_critical(void)
{
    uint32_t primask;
...
/*
 * PendSV_Handler for Cortex-M4 with FPU support.
 *
 ...
 *   Software: R4-R11, S16-S31 (24 words)
 *   Total: 50 words = 200 bytes
 *
